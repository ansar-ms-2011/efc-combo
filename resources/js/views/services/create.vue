<template>
  <div class="p-6 min-h-screen">
    ...
    <div class="text-sm text-gray-600 mb-4">
      ...
      <i class="fa fa-angle-right"></i>
      <span class="cursor-pointer text-blue-600">Add Service</span>
    </div>

    <div class="max-w-7xl mx-auto bg-white rounded-lg shadow border p-6">
      <h2 class="text-xl mb-6 text-black p-2 rounded font-semibold">Add Service</h2>
      ...
      <form @submit.prevent="saveService" class="space-y-6">
        ...
        <div class="flex justify-end gap-3 mt-6">
          <button type="submit" class="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded shadow">Save</button>
...
const saveService = async () => {
  if (!serviceName.value || !selected.value.department) {
  ...
  formData.append('dept_id', selected.value.department.id)
  ...
  try {
    ...
    Swal.fire({ icon: 'success', title: 'Service saved successfully' }).then(() => window.location.href = '/services')
  } catch (err) {
