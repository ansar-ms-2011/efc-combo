    ...
    <div class="page-content">
      ...
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center text-sm text-gray-600 space-x-1">
          ...
          <i class="fa fa-angle-right"></i>
          <span class="font-semibold text-blue-600">Edit {{ currentTypeLabel }}</span>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto bg-white rounded-lg shadow border p-6">
        ...
        <div class="flex items-center gap-2 rounded-t-lg rounded-b-lg mb-4">
          ...
          <h2 class="text-lg font-semibold text-black">Edit {{ currentTypeLabel }}</h2>
        </div>
        ...
        <div class="p-6">
          <form @submit.prevent="handleSubmit">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <!-- Type dropdown, hidden only for COUNTRY -->
...
const currentType = computed(() => route.params.type.toUpperCase())
...
const fetchData = async () => {
  if(id){
    try {
      const res = await axios.get(`/api/demographies/${id}`)
      const data = res.data.data
      form.value.name = data.name
      form.value.urdu_name = data.urdu_name
      form.value.code = data.code
      form.value.type_original = data.type
      form.value.type = arr[data.type]
      form.value.parent_id = data.parent_id
      form.value.is_ajk_district = data.is_ajk_district

      // fetch parents after type is set
      await fetchParents()
...
const handleSubmit = async () => {
  try {
    ...
    await axios.put(`/api/demographies/${id}`, form.value)
    Swal.fire({
      icon: 'success',
      title: 'Good job!',
