...
class StateSubjectPdfJob implements ShouldQueue
{
     ...
     * @throws Throwable
     */
    public function handle(PdfGenerationService $pdfGenerationService)
    {
        ...
        try {
            ...
            $filePaths = $pdfGenerationService->generateSSCPdf($application, $application->application_type_id === 2);

            // Revoke previous certificate if it exists
            if ($application->application_type_id === 2) {
                $previousCertificate = ApplicationCertificate::where('applicant_id', $application->applicant_id)
            ...
            $certificate->update([
                'certificate_number' => "S{$application->region_id}{$application->district_id}{$application->tehsil_id}{$application->center_id}{$certificate->id}"
            ]);
